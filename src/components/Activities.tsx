...
interface ConstraintFilter {
  ...
  positiveValue: 0 | 1  // The value that represents the "good" case
}
...
function ConstraintBadge({
  ...
  // Show colour when value matches the positive case
...
export default function Activities() {
  ...
  // Check if any constraint filters are active
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    // Check constraint filters - filter for the "positive" value when active
          ...
          <div className="p-4 rounded-lg border border-border bg-card mb-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm text-muted-foreground">
                ...
                <a
                  href="/blog/framework-for-evaluating-learning-activities"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredActivities.map((activity) => (
            <ActivityCard
              key={activity.id}
              activity={activity}
              onClick={() => navigate(`/resources/activities/${activity.id}`)}
            />
          ))}
        </div>
      )}
